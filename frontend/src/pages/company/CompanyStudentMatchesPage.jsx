    ...
    useEffect(() => {
        const fetchMatches = async () => {
            try {
                ...
                // For the demo, we fetch matches for a default or first gig
                // In a real app, we'd have a selected gigId
...
{loading ? (
    ...
    matches.map(match => (
        <div key={match.id || match._id} className="bg-surface-container-low rounded-[1rem] p-8 flex items-center gap-8 relative group transition-all duration-300 hover:bg-surface-container-lowest hover:shadow-[0_40px_40px_-15px_rgba(0,24,73,0.06)] hover:-translate-y-1">
            <div className="relative shrink-0 -mt-4 -ml-4">
                <img alt={match.name} className="w-24 h-24 rounded-full object-cover shadow-[0_20px_40px_-10px_rgba(0,24,73,0.15)] ring-4 ring-surface-container-lowest" src={match.imageUrl || `https://ui-avatars.com/api/?name=${match.name}&background=random`}/>
            </div>
