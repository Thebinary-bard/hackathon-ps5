...
export const getMyGigs = asyncHandler(async (req, res) => {
    const gigs = await getMyGigsForUser(req.user.id);
    res.json({ success: true, data: gigs });
});
...
export const apply = asyncHandler(async (req, res) => {
    ...
    res.json({ success: true, message: "Applied successfully", data: gig });
});

export const updateGig = asyncHandler(async (req, res) => {
    const gig = await updateGigById(req.params.id, req.user.id, req.body);
    res.json({ success: true, message: "Gig updated", data: gig });
});

export const updateStatus = asyncHandler(async (req, res) => {
    const gig = await updateGigStatus(req.params.id, req.body.status);
    res.json({ success: true, message: "Status updated", data: gig });
});

export const deleteGig = asyncHandler(async (req, res) => {
    await deleteGigById(req.params.id, req.user.id);
